...
<Note>
  To use Firefox debugging in VS Code, you'll need to install the [Firefox Debugger extension](https://marketplace.visualstudio.com/items?itemName=firefox-devtools.vscode-firefox-debug).
</Note>
...
<Tabs>
  <Tab title="Chrome">
    1. Open Chrome's Developer Tools (`Ctrl+Shift+J` on Windows/Linux, `⌥+⌘+I` on macOS)
    2. Go to the **Sources** tab
    3. Any time your client-side code reaches a `debugger` statement, code execution will pause and that file will appear in the debug area.
    4. To search for files: Press `Ctrl+P` on Windows/Linux or `⌘+P` on macOS
  </Tab>
  <Tab title="Firefox">
    1. Open Firefox's Developer Tools (`Ctrl+Shift+I` on Windows/Linux, `⌥+⌘+I` on macOS)
    2. Go to the **Debugger** tab
    3. Any time your client-side code reaches a `debugger` statement, code execution will pause and that file will appear in the debug area.
    4. To search for files: Press `Ctrl+P` on Windows/Linux or `⌘+P` on macOS, or use the file tree in the left panel
  </Tab>
</Tabs>
...
For React-specific debugging, install the [React Developer Tools](https://react.dev/learn/react-developer-tools) browser extension. This tool helps you:
